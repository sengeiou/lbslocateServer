...
var API_PUSH_BY_DEVICES			   = 'http://openapi.xg.qq.com/v2/push/device_list_multiple';
var API_PUSH_CREATE_MULTIPATH	   = 'http://openapi.xg.qq.com/v2/push/create_multipush';
...
function XingeApp(accessId, secretKey){
	...
	/**
	 * 推送消息给多个设备
	 * @param {string}   deviceList           设备token列表
	 * @param {Message}  message               推送的消息
     * @param {int}      environment           向iOS设备推送时必填，1表示推送生产环境；2表示推送开发环境。Android可不填。
	 * @param {Function} callback(err, result) 回调函数
	 */
	this.pushByDevices = function(deviceList, push_id, callback){
		...
		var params = {
			...
			push_id:Number(push_id)
		};

		callAPI(API_PUSH_BY_DEVICES, params, "POST", 10000, secretKey, callback);
	};

	this.createMultiPush = function(message, callback){
		//效验message合法性
		if(!(message instanceof AndroidMessage) && !(message instanceof IOSMessage)){
			throw new Error('message is invalid');
		}
		...
		callAPI(API_PUSH_CREATE_MULTIPATH, params, "POST", 10000, secretKey, callback);
	};
